...
class Parser:
    ...
    def __init__(self, tokens: list[Token]) -> None:
        ...
        self._declarations: dict[TokenType, Callable[[Token], object]] = {
            ...
            TokenType.DTO: self._parse_dto,
            TokenType.ENUM: self._parse_enum,
            TokenType.TYPE: self._parse_type,
        }
    ...
    def parse(self) -> ProgramNode:
        ...
        while not self._at(TokenType.EOF):
            ...
            elif isinstance(declaration, DtoNode):
                dtos.append(declaration)
            elif isinstance(declaration, EnumNode):
                enums.append(declaration)
            elif isinstance(declaration, TypeNode):
                types.append(declaration)

        return ProgramNode(app, entities, modules, dtos, enums, types, location)
    ...
    def _parse_type(self, keyword: Token) -> TypeNode:
        name = self._consume_identifier("Expected type name")
        return TypeNode(name, self._parse_field_block("type"), keyword.location)
    ...
    def _parse_dto(self, keyword: Token) -> DtoNode:
        name = self._consume_identifier("Expected DTO name")
        self._consume(TokenType.FOR, "Expected 'for' after DTO name")
        entity_name = self._consume_identifier("Expected DTO entity name")
        fields = self._parse_name_block("DTO")
        return DtoNode(name, entity_name, fields, keyword.location)
    ...
    def _parse_module(self, keyword: Token) -> ModuleNode:
        ...
        self._consume(TokenType.LBRACE, "Expected '{' after module entity name")

        routes = []
        while not self._at(TokenType.RBRACE):
            self._reject_nested_declaration("module")
            routes.append(self._parse_route())

        self._consume(TokenType.RBRACE, "Expected '}' after module block")
        return ModuleNode(name, entity_name, routes, keyword.location)
    ...
    def _parse_name_block(self, block_name: str) -> list[str]:
        self._consume(TokenType.LBRACE, f"Expected '{{' after {block_name} header")

        names = []
        while not self._at(TokenType.RBRACE):
            self._reject_nested_declaration()
            names.append(self._consume_identifier(f"Expected {block_name} field name"))
            self._match(TokenType.COMMA)
    ...
    def _parse_route(self) -> RouteNode:
        ...
        self._consume(TokenType.ARROW, "Expected '->' after route path")
        return_type = self._consume_identifier("Expected route return type")
        returns_array = self._parse_optional_array_suffix()

        return RouteNode(method, path, return_type, returns_array, route.location)

    def _parse_route_path(self) -> str:
        self._consume(TokenType.SLASH, "Expected route path starting with '/'")
        parts = ["/"]

        while not self._at(TokenType.ARROW):
            if self._match(TokenType.SLASH):
                parts.append("/")
            elif self._match(TokenType.COLON):
                parameter = self._consume_identifier("Expected path parameter")
                parts.append(f":{parameter}")
            elif self._match(TokenType.IDENT):
                parts.append(self._previous().value)
            else:
                raise self._error(self._peek(), "Expected route path segment")
